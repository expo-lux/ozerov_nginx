...
cmd="curl --connect-timeout 5 \"http://$BASE_DOMAIN/auth/off?key=value\" 2>/dev/null"
...
BASIC_HEADER="$(echo -n user:test | base64)"
cmd="curl --connect-timeout 5 -H \"Authorization: Basic $BASIC_HEADER\" \"http://$BASE_DOMAIN/admin/test1.html?key=value\"-s  -o /dev/null -w '%{http_code}'"
echo "Test 2.3a check $cmd"
res=$(eval $cmd)
if [[ "$res" != "200" ]]; then
    echo "Failed"
    exit 1
...
cmd="curl --connect-timeout 5 \"http://$BASE_DOMAIN/auth/off?key=value\" -o /dev/null -w '%{http_code}'"
echo "Test 2.3b check $cmd"
res=$(eval $cmd)
